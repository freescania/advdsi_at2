...
def split_sets_by_time(df, target_col, test_ratio=0.2):
    ...
    X_train, y_train = subset_x_y(target=target.to_numpy(), features=df_copy.to_numpy(), start_index=0, end_index=-cutoff * 2)
    X_val, y_val = subset_x_y(target=target.to_numpy(), features=df_copy.to_numpy(), start_index=-cutoff * 2, end_index=-cutoff)
    X_test, y_test = subset_x_y(target=target.to_numpy(), features=df_copy.to_numpy(), start_index=-cutoff, end_index=len(target))
